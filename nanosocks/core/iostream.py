...
from nanosocks.core import ioloop
from nanosocks.core import stack_context

import nanosocks.core.protocol as proto
...
class IOStream(object):
    ...
    def __init__(self, socket, io_loop=None, max_buffer_size=104857600,
        ...
        self._read_msg_flag = None
        self._read_flag = None
    ...
    def read_msg(self, callback):
        """Call callback when we read a complete message."""
        assert not self._read_callback, "Already reading"
        self._read_msg_flag = True
        self._read_callback = stack_context.wrap(callback)
        while True:
            if self._read_from_buffer():
                return
    ...
    def read(self, callback, keep_reading=False):
        """Call callback when we read anything."""
        assert not self._read_callback, "Already reading"
        self._read_flag = True
        self._read_callback = stack_context.wrap(callback)
        self._keep_reading = keep_reading
        while True:
            if self._read_from_buffer():
                return
            # self._check_closed()
    ...
    def _read_from_buffer(self):
        ...
        if self._read_bytes:
            ...
            if loc != -1:
                ...
                return True
        elif self._read_msg_flag:
            _merge_prefix(self._read_buffer, sys.maxint)
            if proto.msg_check(self._read_buffer[0]):
                callback = self._read_callback
                self._read_callback = None
                self._read_msg_flag = None
                msg, consume_len = proto.msg_fetchone(self._read_buffer[0])
                self._consume(consume_len)
                self._run_callback(callback, msg)
                return True
        elif self._read_flag:
            _merge_prefix(self._read_buffer, sys.maxint)
            if self._read_buffer[0]:
                callback = self._read_callback
                if not self._keep_reading:
                    self._read_callback = None
                    self._read_flag = None
                self._run_callback(callback,
                                   self._consume(len(self._read_buffer[0])))
                return True
        return False
